...
This folder contains the development container configuration for the Google-Patent-CLI project.
...
3. Configures `tmux`
4. Installs and configures `mise`
5. Runs `mise install` to set up tools
6. Sets up git pre-commit hook
7. Configures `google-patent-cli` to use chromium
8. Authenticates with Z.ai (if `Z_AI_API_KEY` is set)
