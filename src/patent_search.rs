...
impl PatentSearcher {
    ...
    async fn search_internal(&self, options: &SearchOptions) -> Result<Vec<Patent>> {
        ...
        if let Some(patent_number) = &options.patent_number {
            ...
            let results = page
                ...
                (() => {
                    ...
                    return Array.from(items).map(item => {
                        ...
                        const metaEl = item.querySelector(".metadata");
                        const meta = metaEl ? metaEl.innerText.trim() : "";
                        
                        const datesEl = item.querySelector(".dates");
                        const datesText = datesEl ? datesEl.innerText.trim() : "";
                        
                        const abstractEl = item.querySelector(".abstract");
                        const snippet = abstractEl ? abstractEl.innerText.trim() : "";
                        
                        let id = "Unknown";
                        if (titleEl && titleEl.getAttribute("data-result")) {
                            const parts = titleEl.getAttribute("data-result").split("/");
                            if (parts.length > 1) {
                                id = parts[1];
                            }
                        }
            ...
            let patents: Vec<Patent> = serde_json::from_value(results)?;
            Ok(patents)
        }
    }
}
