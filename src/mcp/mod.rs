...
use crate::patent_search::PatentSearcher;
        ...
        match method {
            "tools/list" => {
                let tools = vec![
                    Tool {
                        ...
                        description: "Search Google Patents for patents matching a query"
                            .to_string(),
            ...
            "tools/call" => {
                ...
                match name {
                    "search_patents" => {
                        let query =
                            arguments.get("query").and_then(|v| v.as_str()).map(|s| s.to_string());
                        ...
                        let result = if raw {
                            ...
                            match self.searcher.search(&options).await {
                                Ok(mut results) => results.patents.pop().map_or_else(
                                    ...
                                    |patent| ToolResult {
                                        content: vec![MessageContent::Text {
                                            text: serde_json::to_string_pretty(&patent)
                                                .unwrap_or_default(),
                                        }],
                                        structured_content: None,
                                    },
...
pub async fn run() -> anyhow::Result<()> {
    ...
    let handler = Arc::new(PatentHandler { searcher });
